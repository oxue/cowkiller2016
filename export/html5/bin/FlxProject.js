...
ApplicationMain.preload = function() {
	...
	ApplicationMain.loadFile("assets/images/cow.png");
	ApplicationMain.loadFile("assets/images/darklord.png");
	ApplicationMain.loadFile("assets/images/farmer.png");
	ApplicationMain.loadFile("assets/images/fireball.png");
	ApplicationMain.loadFile("assets/images/g.png");
...
Main.prototype = $extend(flash.display.Sprite.prototype,{
	...
	,setupGame: function() {
		...
		this.addChild(new flixel.FlxGame(this.gameWidth,this.gameHeight,this.initialState,1,this.framerate,this.framerate,this.skipSplash,this.startFullscreen));
	}
	,__class__: Main
});
...
var DefaultAssetLibrary = function() {
	...
	id = "assets/images/darklord.png";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.IMAGE);
	id = "assets/images/farmer.png";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.IMAGE);
...
var FlxLightMap = function(X,Y,_targetTilemap) {
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	flixel.FlxSprite.call(this,X,Y);
	this.scrollFactor = new flixel.util.FlxPoint();
	this.set_blend("MULTIPLY");
	this.targetTilemap = _targetTilemap;
	this.ambientValue = 8947848;
	var s = _targetTilemap.getTileSize();
	this.lightMapW = flixel.FlxG.width + s.x;
	this.lightMapH = flixel.FlxG.height + s.y;
	this.lightMap = new tgame.Array2D(this.lightMapW / s.x,this.lightMapH / s.x,-11184811);
	this.makeGraphic(this.lightMapW / s.x | 0,this.lightMapH / s.x | 0,-256);
	this.scale.set_x(this.scale.set_y(8));
	this.origin.set_x(this.origin.set_y(0));
};
$hxClasses["FlxLightMap"] = FlxLightMap;
FlxLightMap.__name__ = ["FlxLightMap"];
FlxLightMap.__super__ = flixel.FlxSprite;
FlxLightMap.prototype = $extend(flixel.FlxSprite.prototype,{
	targetTilemap: null
	,ambientValue: null
	,lightMap: null
	,lightMapW: null
	,lightMapH: null
	,blitLight_add: function(_light) {
		var r = new flash.geom.Rectangle(_light.x - _light.radius,_light.y - _light.radius,_light.computedLightmap.component.width,_light.computedLightmap.component.height);
		var vs = this.get_pixels().getPixels(r);
		vs.position = 0;
		var dv = _light.computedLightmap.getPixels(_light.computedLightmap.rect);
		var i = vs.length;
		while(i-- > 0) vs[i] += dv[i];
		this.get_pixels().setPixels(r,vs);
	}
	,draw: function() {
		var pos = this.targetTilemap.getScreenXY();
		var ts = this.targetTilemap.getTileSize();
		this.set_x(pos.x % ts.x);
		this.set_y(pos.y % ts.y);
		var tileX = -pos.x / ts.x | 0;
		var tileY = -pos.y / ts.x | 0;
		var i = Std["int"](this.lightMap.getWidth());
		while(i-- > 0) {
			var xv = tileX + i;
			var ambCD = 5;
			var j = 0;
			while(j < this.lightMap.getHeight()) {
				var yv = tileY + j;
				if(this.targetTilemap.getTile(xv,yv) == 0) this.get_pixels().setPixel(i,j,this.ambientValue); else {
					if(ambCD > 0) ambCD--;
					this.get_pixels().setPixel(i,j,3289650 + ambCD * 2631720);
				}
				j++;
			}
		}
		var _g = 0;
		var _g1 = tgame.World.curWorld.lightSources;
		while(_g < _g1.length) {
			var l = _g1[_g];
			++_g;
			this.blitLight_add(l);
		}
		flixel.FlxSprite.prototype.draw.call(this);
	}
	,__class__: FlxLightMap
});
flixel.group = {};
...
var FlxTrail = function(Sprite,Image,Length,Delay,Alpha,Diff) {
	...
	this.recentAngles = new Array();
	this.recentPositions = new Array();
	this.sprite = Sprite;
	this.delay = Delay;
	this.image = Image;
	this.transp = Alpha;
...
$hxClasses["FlxTrail"] = FlxTrail;
FlxTrail.__name__ = ["FlxTrail"];
FlxTrail.__super__ = flixel.group.FlxGroup;
FlxTrail.prototype = $extend(flixel.group.FlxGroup.prototype,{
	sprite: null
	,delay: null
	,xEnabled: null
	,yEnabled: null
	...
	,postUpdate: function() {
		this.counter++;
		if(this.counter >= this.delay && this.trailLength >= 1) {
			this.counter = 0;
			var spritePosition = new flixel.util.FlxPoint(this.sprite.x - this.sprite.offset.x,this.sprite.y - this.sprite.offset.y);
			this.recentPositions.unshift(spritePosition);
			if(this.recentPositions.length > this.trailLength) this.recentPositions.pop();
			if(this.rotationsEnabled) {
				var spriteAngle = this.sprite.angle;
				this.recentAngles.unshift(spriteAngle);
				if(this.recentAngles.length > this.trailLength) this.recentAngles.pop();
			}
			var trailSprite;
			var _g1 = 0;
			var _g = this.recentPositions.length;
			while(_g1 < _g) {
				var i = _g1++;
				trailSprite = this.members[i];
				trailSprite.set_x(this.recentPositions[i].x);
				trailSprite.set_y(this.recentPositions[i].y);
				if(this.rotationsEnabled) trailSprite.set_angle(this.recentAngles[i]);
				trailSprite.set_exists(true);
			}
		}
		flixel.group.FlxGroup.prototype.update.call(this);
	}
	,resetTrail: function() {
		this.recentPositions.splice(0,this.recentPositions.length);
		this.recentAngles.splice(0,this.recentAngles.length);
		var _g1 = 0;
		var _g = this.members.length;
		while(_g1 < _g) {
			var i = _g1++;
			this.members[i].set_exists(false);
		}
	}
	,increaseLength: function(amount) {
		...
		while(_g < amount) {
			var i = _g++;
			var trailSprite = new flixel.FlxSprite(0,0,this.image);
			trailSprite.set_exists(false);
			this.add(trailSprite);
			trailSprite.set_alpha(this.transp);
			this.transp -= this.difference;
	...
	,changeGraphic: function(Image) {
		this.image = Image;
		var _g1 = 0;
		var _g = this.trailLength;
		while(_g1 < _g) {
			var i = _g1++;
			(js.Boot.__cast(this.members[i] , flixel.FlxSprite)).loadGraphic(Image);
		}
	}
	,changeValuesEnabled: function(Angle,X,Y) {
		if(Y == null) Y = true;
		if(X == null) X = true;
...
$hxClasses["GameConstants"] = GameConstants;
GameConstants.__name__ = ["GameConstants"];
GameConstants.prototype = {
	__class__: GameConstants
};
...
MenuState.prototype = $extend(flixel.FlxState.prototype,{
	...
	,create: function() {
		...
		this.focus = new flixel.FlxObject(320,240);
		tgame.FlxTerraGen.initRand();
		this.set_bgColor(-2490369);
		this.world = new tgame.World();
		tgame.World.curWorld = this.world;
		this.world.tilemap = this.add(tgame.FlxTerraGen.generateTerramap(1024,128));
		var d = new tgame.critters.DarkLord(100,100);
		this.world.solids.add(d);
		this.world.friendlyCritters.add(d);
		d.addBehavior(new tgame.behaviors.WASDControl());
		d.addBehavior(new tgame.behaviors.DarkLordControl());
		this.add(d);
		var i = 20;
		while(i-- > 0) {
			var c = new tgame.critters.Cow(0,0);
			c.reset(100 * i,10);
			this.world.solids.add(c);
			this.world.enemyCritters.add(c);
			this.world.friendlyCritters.add(c);
			this.add(c);
			var f = new tgame.critters.Human(10 + 50 * i,10);
			this.world.solids.add(f);
			this.world.enemyCritters.add(f);
			this.world.friendlyCritters.add(f);
			this.add(f);
			f.addBehavior(new tgame.behaviors.WanderBehavior(200,300));
		}
		i = 5;
		while(i-- > 0) {
			var dling = new tgame.critters.DarkLord(d.x + 200 + i * 150,d.y);
			dling.addBehavior(new tgame.behaviors.WanderBehavior(30,50));
			dling.addBehavior(new tgame.behaviors.EnemyDarkLord());
			this.world.solids.add(dling);
			this.world.enemyCritters.add(dling);
			this.add(dling);
		}
		this.world.lm = new FlxLightMap(0,0,this.world.tilemap);
		this.l = new tgame.LightSource(20,20,16777096,3);
		this.world.lightSources.push(this.l);
		var dia = new tgame.DialogueBox();
		dia.setText("ASDAASDASDA");
		flash.Lib.get_current().get_stage().addChild(dia);
		flixel.FlxG.camera.follow(this.focus);
		flixel.FlxG.worldBounds.width = this.world.tilemap.width;
		flixel.FlxG.worldBounds.height = this.world.tilemap.height;
		this.hud = new tgame.HUD(d);
		this.dark = d;
	}
	,destroy: function() {
	...
	,update: function() {
		if(flixel.FlxG.mouse.wheel != 0) {
			haxe.Log.trace(flixel.FlxG.camera.zoom,{ fileName : "MenuState.hx", lineNumber : 135, className : "MenuState", methodName : "update"});
			var _g = flixel.FlxG.camera;
			_g.set_zoom(_g.zoom + flixel.FlxG.mouse.wheel / Math.abs(flixel.FlxG.mouse.wheel) * 0.02);
			flixel.FlxG.camera.set_width(640 / flixel.FlxG.camera.zoom);
			flixel.FlxG.camera.set_height(480 / flixel.FlxG.camera.zoom);
		}
		if(flixel.FlxG.keys.checkStatus(37,flixel.FlxG.keys.pressed.checkStatus)) {
			var _g1 = this.focus;
			_g1.set_x(_g1.x - this.speed);
		}
		if(flixel.FlxG.keys.checkStatus(39,flixel.FlxG.keys.pressed.checkStatus)) {
			var _g2 = this.focus;
			_g2.set_x(_g2.x + this.speed);
		}
		if(flixel.FlxG.keys.checkStatus(38,flixel.FlxG.keys.pressed.checkStatus)) {
			var _g3 = this.focus;
			_g3.set_y(_g3.y - this.speed);
		}
		if(flixel.FlxG.keys.checkStatus(40,flixel.FlxG.keys.pressed.checkStatus)) {
			var _g4 = this.focus;
			_g4.set_y(_g4.y + this.speed);
		}
		flixel.FlxState.prototype.update.call(this);
		this.world.overlap();
		this.l.set_x(this.dark.getScreenXY().x / 8);
		this.l.set_y(this.dark.getScreenXY().y / 8);
		haxe.Log.trace(this.l.x,{ fileName : "MenuState.hx", lineNumber : 156, className : "MenuState", methodName : "update"});
		haxe.Log.trace(this.l.y,{ fileName : "MenuState.hx", lineNumber : 157, className : "MenuState", methodName : "update"});
		this.hud.update();
	}
	,__class__: MenuState
});
...
flixel.tile.FlxTilemap.prototype = $extend(flixel.FlxObject.prototype,{
	...
	,getTileSize: function() {
		return new flixel.util.FlxPoint(this._tileWidth,this._tileHeight);
	}
	,overlaps: function(ObjectOrGroup,InScreenSpace,Camera) {
		if(InScreenSpace == null) InScreenSpace = false;
		var group = flixel.group.FlxGroup.resolveGroup(ObjectOrGroup);
...
var tgame = {};
tgame.Array2D = function(_w,_h,_defVal) {
	var len = _w * _h;
	var _g = [];
	var _g1 = 0;
	while(_g1 < len) {
...
$hxClasses["tgame.Array2D"] = tgame.Array2D;
tgame.Array2D.__name__ = ["tgame","Array2D"];
tgame.Array2D.prototype = {
	data: null
	,width: null
	,height: null
	,get: function(_x,_y) {
		return this.data[_y * this.width + _x];
	}
	,set: function(_x,_y,_val) {
		this.data[_y * this.width + _x] = _val;
	}
	,getWidth: function() {
		return this.width;
	}
	...
	,__class__: tgame.Array2D
};
tgame.Behavior = function() {
};
$hxClasses["tgame.Behavior"] = tgame.Behavior;
tgame.Behavior.__name__ = ["tgame","Behavior"];
tgame.Behavior.prototype = {
	update: function(_critter) {
	}
	,__class__: tgame.Behavior
};
tgame.DialogueBox = function() {
	flash.display.Sprite.call(this);
	this.get_graphics().beginFill(2236962);
	this.get_graphics().drawRect(0,0,200,300);
	this.textBox = new flash.text.TextField();
	this.textBox.set_x(this.textBox.set_y(10));
	this.textBox.set_width(180);
	this.textBox.set_height(100);
	this.textBox.set_selectable(false);
	this.textBox.defaultTextFormat = new flash.text.TextFormat("arial",12,268435455);
	this.addChild(this.textBox);
	this.xButton = new flash.display.Sprite();
	this.xButton.get_graphics().beginFill(3355443);
	this.xButton.get_graphics().drawRect(0,0,10,10);
	this.xButton.get_graphics().lineStyle(2,16711680);
	this.xButton.get_graphics().moveTo(0,0);
	this.xButton.get_graphics().lineTo(10,10);
	this.xButton.get_graphics().moveTo(0,10);
	this.xButton.get_graphics().lineTo(10,0);
	this.xButton.set_x(190);
	this.xButton.addEventListener("mouseClick",$bind(this,this.destroy));
	this.addChild(this.xButton);
};
$hxClasses["tgame.DialogueBox"] = tgame.DialogueBox;
tgame.DialogueBox.__name__ = ["tgame","DialogueBox"];
tgame.DialogueBox.__super__ = flash.display.Sprite;
tgame.DialogueBox.prototype = $extend(flash.display.Sprite.prototype,{
	textBox: null
	,xButton: null
	,destroy: function(e) {
		this.xButton.removeEventListener("mouseClick",$bind(this,this.destroy));
		this.parent.removeChild(this);
	}
	,setText: function(_msg) {
		this.textBox.set_text(_msg);
		this.textBox.set_height(this.textBox.get_textHeight());
	}
	,__class__: tgame.DialogueBox
});
tgame.Random = function(seed) {
	if(seed == null) seed = 1;
	this.seed = seed;
};
$hxClasses["tgame.Random"] = tgame.Random;
tgame.Random.__name__ = ["tgame","Random"];
tgame.Random.prototype = {
	seed: null
	,'int': function() {
	...
	,__class__: tgame.Random
};
tgame.FlxTerraGen = function() {
};
$hxClasses["tgame.FlxTerraGen"] = tgame.FlxTerraGen;
tgame.FlxTerraGen.__name__ = ["tgame","FlxTerraGen"];
tgame.FlxTerraGen.generateTerramap = function(_mapWidth,_mapHeight) {
	tgame.FlxTerraGen.rand = new tgame.Random(33);
	var ret = new flixel.tile.FlxTilemap();
	ret.widthInTiles = _mapWidth;
	ret.heightInTiles = _mapHeight;
	var mapData = new tgame.Array2D(_mapWidth,_mapHeight,0);
	var heights = tgame.FlxTerraGen.generateHeightArray(_mapWidth);
	var i = _mapWidth;
	while(i-- > 0) {
		var hVal;
		var constHVal = hVal = Std["int"](heights[i] * js.Boot.__cast(_mapHeight , Float));
		if(hVal <= 0) hVal = 1;
		var j = _mapHeight;
		while(hVal-- > 0) {
			j--;
			if(j > _mapHeight - constHVal + Math.sin(i * 0.05) * 4 + 6) mapData.set(i,j,2); else mapData.set(i,j,1);
		}
	}
	ret.loadMap(mapData.getInternalData(),openfl.Assets.getBitmapData("assets/images/tiles.png",true),8,8,0);
	return ret;
};
tgame.FlxTerraGen.randBetween = function(_a,_b) {
	return _a + tgame.FlxTerraGen.rand["float"]() * (_b - _a);
};
tgame.FlxTerraGen.generateHeightArray = function(_w) {
	var nodes = [0.4,0.4 + tgame.FlxTerraGen.rand["float"]() * 0.099999999999999978,0.4];
	var randMax = 0.6;
	while(nodes.length < _w) {
		...
		while(i-- > 1) {
			...
			var x = med + tgame.FlxTerraGen.randBetween(-randMax,randMax);
			nodes.splice(i,0,x);
			randMax *= 0.95;
		}
	}
...
tgame.FlxTerraGen.initRand = function() {
	tgame.FlxTerraGen.rand = new tgame.Random(100);
};
tgame.FlxTerraGen.prototype = {
	__class__: tgame.FlxTerraGen
};
tgame.HUD = function(_target) {
	this.target = _target;
	this.healthBar = new tgame.ValueBar(100);
	flixel.FlxG.game._state.add(this.healthBar);
};
$hxClasses["tgame.HUD"] = tgame.HUD;
tgame.HUD.__name__ = ["tgame","HUD"];
tgame.HUD.prototype = {
	target: null
	,healthBar: null
	,update: function() {
		this.healthBar.setValue(this.target.health);
	}
	,__class__: tgame.HUD
};
tgame.IHazardous = function() { };
$hxClasses["tgame.IHazardous"] = tgame.IHazardous;
tgame.IHazardous.__name__ = ["tgame","IHazardous"];
tgame.IHazardous.prototype = {
	damage: null
	,oneTime: null
	,__class__: tgame.IHazardous
};
tgame.LightSource = function(X,Y,_color,_radius) {
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	flixel.util.FlxPoint.call(this,X,Y);
	this.radius = _radius;
	this.computedLightmap = new flash.display.BitmapData(this.radius * 2 + 1,this.radius * 2 + 1,false);
	var w = this.computedLightmap.component.width;
	var h = this.computedLightmap.component.height;
	var inverseRSq = 1 / (this.radius * this.radius);
	while(w-- > 0) {
		var j = h;
		while(j-- > 0) {
			var dx = w - this.radius;
			var dy = j - this.radius;
			var fval = 1 - (dx * dx + dy * dy) * inverseRSq;
			var c = -16777216 | ((_color >> 16 & 255) * fval | 0) << 16 | ((_color >> 8 & 255) * fval | 0) << 8 | ((_color & 255) * fval | 0);
			this.computedLightmap.setPixel(w,j,c);
		}
	}
};
$hxClasses["tgame.LightSource"] = tgame.LightSource;
tgame.LightSource.__name__ = ["tgame","LightSource"];
tgame.LightSource.__super__ = flixel.util.FlxPoint;
tgame.LightSource.prototype = $extend(flixel.util.FlxPoint.prototype,{
	computedLightmap: null
	,radius: null
	,__class__: tgame.LightSource
});
tgame.ValueBar = function(_maxVal) {
	flixel.FlxSprite.call(this,this.x,this.y);
	this.barWidth = 100;
	this.value = this.maxValue = _maxVal;
	this.origin.set_x(this.origin.set_y(0));
	this.makeGraphic(1,10,-65536);
	this.scrollFactor.set_x(this.scrollFactor.set_y(0));
	this.scale.set_x(this.barWidth);
};
$hxClasses["tgame.ValueBar"] = tgame.ValueBar;
tgame.ValueBar.__name__ = ["tgame","ValueBar"];
tgame.ValueBar.__super__ = flixel.FlxSprite;
tgame.ValueBar.prototype = $extend(flixel.FlxSprite.prototype,{
	barWidth: null
	,value: null
	,maxValue: null
	,setValue: function(_val) {
		this.scale.set_x(_val / this.maxValue * this.barWidth);
		this.value = _val;
	}
	,__class__: tgame.ValueBar
});
tgame.World = function() {
	this.friendlyCritters = new flixel.group.FlxGroup();
	this.friendlyHazards = new flixel.group.FlxGroup();
	this.enemyCritters = new flixel.group.FlxGroup();
	this.enemyHazards = new flixel.group.FlxGroup();
	this.solids = new flixel.group.FlxGroup();
	this.lightSources = new Array();
};
$hxClasses["tgame.World"] = tgame.World;
tgame.World.__name__ = ["tgame","World"];
tgame.World.prototype = {
	friendlyCritters: null
	,friendlyHazards: null
	,enemyCritters: null
	,enemyHazards: null
	,solids: null
	,smoke: null
	...
	,lm: null
	,lightSources: null
	,prepareCows: function(_num) {
		while(_num-- > 0) this.friendlyCritters.add(new tgame.critters.Cow(0,0)).kill();
	}
	,overlap: function() {
		flixel.FlxG.overlap(this.tilemap,this.solids,null,flixel.FlxObject.separate);
		flixel.FlxG.overlap(this.friendlyHazards,this.friendlyCritters,$bind(this,this.hurtCritter));
		flixel.FlxG.overlap(this.enemyHazards,this.enemyCritters,$bind(this,this.hurtCritter));
	}
	,hurtCritter: function(_b,_c) {
		_b.damage(_c);
		if(_b.oneTime()) _b.destroy();
	}
	,__class__: tgame.World
};
tgame.behaviors = {};
tgame.behaviors.DarkLordControl = function() {
	tgame.Behavior.call(this);
};
$hxClasses["tgame.behaviors.DarkLordControl"] = tgame.behaviors.DarkLordControl;
tgame.behaviors.DarkLordControl.__name__ = ["tgame","behaviors","DarkLordControl"];
tgame.behaviors.DarkLordControl.__super__ = tgame.Behavior;
tgame.behaviors.DarkLordControl.prototype = $extend(tgame.Behavior.prototype,{
	update: function(_critter) {
		var d = _critter;
		if(flixel.FlxG.keys.checkStatus(16,flixel.FlxG.keys.justPressed.checkStatus)) d.dash();
		if(flixel.FlxG.keys.checkStatus(81,flixel.FlxG.keys.justPressed.checkStatus)) d.fireBall(flixel.FlxG.mouse.x,flixel.FlxG.mouse.y);
	}
	,__class__: tgame.behaviors.DarkLordControl
});
tgame.behaviors.EnemyDarkLord = function() {
	tgame.Behavior.call(this);
	this.timer = 400;
	this.time = 0;
};
$hxClasses["tgame.behaviors.EnemyDarkLord"] = tgame.behaviors.EnemyDarkLord;
tgame.behaviors.EnemyDarkLord.__name__ = ["tgame","behaviors","EnemyDarkLord"];
tgame.behaviors.EnemyDarkLord.__super__ = tgame.Behavior;
tgame.behaviors.EnemyDarkLord.prototype = $extend(tgame.Behavior.prototype,{
	time: null
	,timer: null
	,update: function(_critter) {
		var d;
		d = js.Boot.__cast(_critter , tgame.critters.DarkLord);
		++this.time;
		if(this.time >= this.timer) {
			d.dash();
			this.time -= Math.random() * 300;
			d.fireBall(d.x + Math.random() * 10 - 5,d.y - Math.random() * 5,2);
		}
	}
	,__class__: tgame.behaviors.EnemyDarkLord
});
tgame.behaviors.WASDControl = function() {
	tgame.Behavior.call(this);
};
$hxClasses["tgame.behaviors.WASDControl"] = tgame.behaviors.WASDControl;
tgame.behaviors.WASDControl.__name__ = ["tgame","behaviors","WASDControl"];
tgame.behaviors.WASDControl.__super__ = tgame.Behavior;
tgame.behaviors.WASDControl.prototype = $extend(tgame.Behavior.prototype,{
	update: function(_critter) {
		if(flixel.FlxG.keys.checkStatus(65,flixel.FlxG.keys.pressed.checkStatus)) _critter.moveToward(1); else if(flixel.FlxG.keys.checkStatus(68,flixel.FlxG.keys.pressed.checkStatus)) _critter.moveToward(2); else _critter.moveToward(0);
		if(flixel.FlxG.keys.checkStatus(87,flixel.FlxG.keys.justPressed.checkStatus)) _critter.jump();
	}
	,__class__: tgame.behaviors.WASDControl
});
tgame.behaviors.WanderBehavior = function(_timeMin,_timeMax) {
	this.timer = tgame.FlxTerraGen.randBetween(_timeMin,_timeMax);
...
$hxClasses["tgame.behaviors.WanderBehavior"] = tgame.behaviors.WanderBehavior;
tgame.behaviors.WanderBehavior.__name__ = ["tgame","behaviors","WanderBehavior"];
tgame.behaviors.WanderBehavior.__super__ = tgame.Behavior;
tgame.behaviors.WanderBehavior.prototype = $extend(tgame.Behavior.prototype,{
	time: null
	,timer: null
	,timeMin: null
	,timeMax: null
	,update: function(_critter) {
		if((_critter.touching & 1) > 0 || (_critter.touching & 16) > 0) _critter.jump();
		this.time += 1;
		if(this.time >= this.timer) {
			this.time = 0;
			this.timer = tgame.FlxTerraGen.randBetween(this.timeMin,this.timeMax);
			var r = Math.random();
			_critter.moveToward(r < 0.2?1:r < 0.4?2:0);
		}
	}
	,__class__: tgame.behaviors.WanderBehavior
});
tgame.critters = {};
tgame.critters.Critter = function(X,Y,_graphic) {
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	flixel.FlxSprite.call(this,X,Y);
	this.behaviors = [];
	...
	this.set_solid(true);
	if(_graphic != null) this.loadGraphic(_graphic);
	this._facingFlip.set(16,{ x : false, y : false});
	this._facingFlip.set(1,{ x : true, y : false});
	this.setSpeed(1);
	this.setJumpPower(160);
};
$hxClasses["tgame.critters.Critter"] = tgame.critters.Critter;
tgame.critters.Critter.__name__ = ["tgame","critters","Critter"];
tgame.critters.Critter.__super__ = flixel.FlxSprite;
tgame.critters.Critter.prototype = $extend(flixel.FlxSprite.prototype,{
	speed: null
	,jumpPower: null
	,behaviors: null
	,hurt: function(Damage) {
		flixel.FlxSprite.prototype.hurt.call(this,Damage);
	}
	,addBehavior: function(_beh) {
		this.behaviors.push(_beh);
	}
	,getSpeed: function() {
		return this.speed;
	}
	...
	,moveToward: function(_flag) {
		...
		if(_flag == 0) this.acceleration.set_x(0);
	}
	,getJumpPower: function() {
		return this.jumpPower;
	}
	,setJumpPower: function(_power) {
		this.jumpPower = _power;
	}
	,jump: function(_factor) {
		if(_factor == null) _factor = 1;
		if((this.touching & 4096) > 0) this.velocity.set_y(-this.jumpPower * _factor);
	}
	,update: function() {
		var _g = 0;
		...
		if(flixel.effects.FlxFlicker.isFlickering(this)) this.set_color(16742263); else this.set_color(16777215);
		flixel.FlxSprite.prototype.update.call(this);
	}
	,__class__: tgame.critters.Critter
});
tgame.critters.Cow = function(_x,_y) {
	tgame.critters.Critter.call(this,_x,_y,openfl.Assets.getBitmapData("assets/images/cow.png",true));
	this.setJumpPower(160);
	this.setSpeed(25);
	this.health = 5;
	this.addBehavior(new tgame.behaviors.WanderBehavior(100,200));
};
$hxClasses["tgame.critters.Cow"] = tgame.critters.Cow;
tgame.critters.Cow.__name__ = ["tgame","critters","Cow"];
tgame.critters.Cow.__super__ = tgame.critters.Critter;
tgame.critters.Cow.prototype = $extend(tgame.critters.Critter.prototype,{
	reset: function(X,Y) {
		this.health = 5;
		this.revive();
		tgame.critters.Critter.prototype.reset.call(this,X,Y);
	}
	,__class__: tgame.critters.Cow
});
tgame.critters.DarkLord = function(X,Y) {
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	tgame.critters.Critter.call(this,X,Y,openfl.Assets.getBitmapData("assets/images/darklord.png"));
	this.setSpeed(50);
	this.setJumpPower(180);
	this.trail = new FlxTrail(this,this.get_pixels(),5,2,0.6,0.02);
	flixel.FlxG.game._state.add(this.trail);
	this.dashing = false;
	this.dashTime = 0;
	this.dashTotal = 20;
	this.dashSpeed = 500;
...
$hxClasses["tgame.critters.DarkLord"] = tgame.critters.DarkLord;
tgame.critters.DarkLord.__name__ = ["tgame","critters","DarkLord"];
tgame.critters.DarkLord.__super__ = tgame.critters.Critter;
tgame.critters.DarkLord.prototype = $extend(tgame.critters.Critter.prototype,{
	trail: null
	,dashing: null
	,dashTime: null
	,dashTotal: null
	,dashSpeed: null
	,update: function() {
		tgame.critters.Critter.prototype.update.call(this);
		if(this.dashTime > 5) {
			this.dashTime--;
			this.velocity.set_x(this.dashSpeed * (this.facing == 1?-1:1));
			this.velocity.set_y(0);
		} else if(this.dashTime > 0) this.dashTime--; else {
			this.acceleration.set_y(600);
			this.trail.set_exists(false);
			this.dashing = false;
		}
		if(Math.abs(this.velocity.x) > 450) this.trail.set_exists(true);
		this.trail.postUpdate();
	}
	,fireBall: function(_x,_y,_aff) {
		if(_aff == null) _aff = 1;
		var f = new tgame.projectile.Fireball(this.x,this.y,new flixel.util.FlxPoint(_x - this.x,_y - this.y));
		flixel.FlxG.game._state.add(f);
		if(_aff == 1) tgame.World.curWorld.enemyHazards.add(f);
		if(_aff == 2) tgame.World.curWorld.friendlyHazards.add(f);
	}
	,dash: function() {
		if(this.dashing) return;
		this.trail.set_exists(true);
		this.dashTime = this.dashTotal;
		this.dashing = true;
	}
	,__class__: tgame.critters.DarkLord
});
tgame.critters.Human = function(X,Y,_graphic) {
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	tgame.critters.Critter.call(this,X,Y,openfl.Assets.getBitmapData("assets/images/farmer.png",true));
	this.health = 20;
	this.setSpeed(20);
};
$hxClasses["tgame.critters.Human"] = tgame.critters.Human;
tgame.critters.Human.__name__ = ["tgame","critters","Human"];
tgame.critters.Human.__super__ = tgame.critters.Critter;
tgame.critters.Human.prototype = $extend(tgame.critters.Critter.prototype,{
	__class__: tgame.critters.Human
});
tgame.projectile = {};
tgame.projectile.Fireball = function(_x,_y,_vel) {
	flixel.FlxSprite.call(this,_x,_y,openfl.Assets.getBitmapData("assets/images/fireball.png",true));
	this.speed = 200;
	this.acceleration.set_y(100);
	var dis = _vel.distanceTo(new flixel.util.FlxPoint());
	var _g = _vel;
	_g.set_x(_g.x / dis);
	var _g1 = _vel;
	_g1.set_y(_g1.y / dis);
	...
	this.decaying = false;
	this.decayTime = 80;
	this.smoke = new tgame.projectile.Smoke();
	this.smoke.start(false,1,0.1);
	this.smoke.setXSpeed(-30,30);
	this.smoke.setYSpeed(-30,15);
};
$hxClasses["tgame.projectile.Fireball"] = tgame.projectile.Fireball;
tgame.projectile.Fireball.__name__ = ["tgame","projectile","Fireball"];
tgame.projectile.Fireball.__interfaces__ = [tgame.IHazardous];
tgame.projectile.Fireball.__super__ = flixel.FlxSprite;
tgame.projectile.Fireball.prototype = $extend(flixel.FlxSprite.prototype,{
	speed: null
	,smoke: null
	,decaying: null
	,decayTime: null
	...
	,update: function() {
		...
		this.smoke.at(this);
		this.smoke.update();
		if(this.overlaps(tgame.World.curWorld.tilemap)) this.destroy();
	}
	,destroy: function() {
		this.decaying = true;
	...
	,damage: function(_c) {
		if(!this.decaying) {
			flixel.effects.FlxFlicker.flicker(_c,1);
			_c.hurt(10);
			if(js.Boot.__instanceof(_c,tgame.critters.Critter)) (js.Boot.__cast(_c , tgame.critters.Critter)).jump(0.5);
		}
	}
	,draw: function() {
		this.smoke.draw();
		if(!this.decaying) flixel.FlxSprite.prototype.draw.call(this);
	}
	,__class__: tgame.projectile.Fireball
});
tgame.projectile.Smoke = function(X,Y,Size) {
	if(Size == null) Size = 0;
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	flixel.effects.particles.FlxEmitter.call(this,X,Y,10);
	this.makeParticles(new flixel.FlxSprite().makeGraphic(20,20,-1725816286).get_pixels(),10,0,false,0);
};
$hxClasses["tgame.projectile.Smoke"] = tgame.projectile.Smoke;
tgame.projectile.Smoke.__name__ = ["tgame","projectile","Smoke"];
tgame.projectile.Smoke.__super__ = flixel.effects.particles.FlxEmitter;
tgame.projectile.Smoke.prototype = $extend(flixel.effects.particles.FlxEmitter.prototype,{
	__class__: tgame.projectile.Smoke
});
...
GameConstants.DIALOGUE_BOX_W = 200;
GameConstants.DIALOGUE_BOX_H = 300;
...
tgame.critters.Critter.LEFT = 1;
tgame.critters.Critter.RIGHT = 2;
tgame.critters.Critter.stdGrav = 600;
